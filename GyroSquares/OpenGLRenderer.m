...
@implementation OpenGLRenderer
{
    ...
    GLint m_squareMvpUniformIdx;
    GLuint m_squareVAOName;
    GLuint m_squareTexName;
...
- (void)render {
	...
	mtxLoadPerspective(projection, 65, (float)m_viewWidth / (float)m_viewHeight,1.0,100.0);
   ...
   // mtxRotateXApply(modelView, -45.0);
	...
	glUniformMatrix4fv(m_squareMvpUniformIdx, 1, GL_FALSE, mvp);
...
- (id)initWithDefaultFBO: (GLuint) defaultFBOName
{
	if((self = [super init]))
	{
		...
		m_squareMvpUniformIdx = glGetUniformLocation(m_squarePrgName, "modelViewProjectionMatrix");
		
		if(m_squareMvpUniformIdx < 0)
		{
